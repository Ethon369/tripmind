<template>
  <section class="hp">
    <a-collapse v-model:activeKey="activeKeys" :bordered="false" class="hp-collapse">
      <!-- ---------------- 三层知识库 ---------------- -->
      <a-collapse-panel key="layers" header="三层知识库各回答什么问题">
        ...
        <p class="hp-note">
          前两层是「为什么需要 RAG」的正当答案 —— 门票涨价、闭馆日、预约规则这类信息，
          模型最容易记错或过时。POI 层补的是另一个缺口：高德的文字检索只返回名字和地址、
          <b>不返回坐标</b>，模型手上没有坐标就容易编。
        </p>
        <p class="hp-note">
          第三层（上传）是最能说明 RAG 价值的一层：<b>你自己的攻略模型不可能知道</b>。
          上传后每一块都会带着出处 —— 生成行程时被引用的那一段，
          在行程详情页里能一直追溯到「你上传的哪篇攻略的哪一节」。
        </p>
        <p class="hp-note">
          三层用<b>同一个</b> embedding 模型和同一种度量，所以分数可以直接比，
          归并排序才公平。上传内容单独一层而不是混进内置攻略，是因为
          「重灌内置库」会清空整个集合 —— 混在一起的话，你传的攻略会被一起删掉。
        </p>
      </a-collapse-panel>
      ...
      <a-collapse-panel key="enable" header="ENABLE_RAG 开关与评测的关系">
        ...
        <p class="hp-note">
          现在有两种打开方式：<b>上传攻略时自动打开</b>（用户点「入库」这个动作本身
          就是在表达"我想让它参与生成"，再让他去别处开开关是多余步骤），
          或者在本页顶部的提示条里点「立即开启」。
        </p>
        <p class="hp-note">
          这两种都是<b>运行时开关</b>：只改当前进程的状态，<b>不写回 .env</b>，
          重启后恢复原值。之所以不落盘，是因为 .env 是评测基线的唯一来源 ——
          让日常操作去改它，下次跑评测时基线就已经被污染了。
          想永久打开，仍在 <code>backend/.env</code> 里写 <code>ENABLE_RAG=true</code> 后重启。
        </p>
      </a-collapse-panel>
    </a-collapse>
  </section>
