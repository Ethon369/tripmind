...
#
# ⚠️ 换算时踩过的坑:官方范围常用**度分**(如 "34°45′"),不是小数。
# `34°45′` 是 34.75,不是 34.45。本文件最初把西安的四个值都按小数读了,
# 框比实际小了 —— 结果西安 7 个合法的 POI(阎良区、高陵区、蓝田县)
# 被 coord_coverage 判成"越界"。**这是假警报,比指标偏低更糟。**
#
# 现在每个值都对着实测算过(见 tests/test_geo.py 里用录制库存做的回归测试)。
# 已用真实数据验证过的 5 个城市:北京 / 上海 / 成都 / 西安 / 杭州;其余待验证。
CITY_BBOX: dict[str, tuple[float, float, float, float]] = {
    "北京": (115.42, 39.44, 117.52, 41.08),
    "上海": (120.85, 30.67, 122.20, 31.88),
    "成都": (102.54, 30.05, 104.88, 31.44),
    # 107°40′~109°49′E, 33°42′~34°45′N —— 注意是**度分**,换算后如下
