...
- **账号与角色**：账号密码登录 + 自助注册（管理员 / 普通用户两种角色）。普通用户只看得到自己的历史行程与**自己上传的攻略**；管理员可管理账号、查看全部用户的行程、灌内置知识库
- **落地页**：公开的产品介绍页（打开站点看到的第一个页面），滚动时各模块缓慢推出；「免费开始」直接注册
- **知识库（可选）**：**每个账号有自己的上传层**（只有本人和管理员看得到，检索时也只带自己的）；上传攻略入库——支持 Markdown / 纯文本 / PDF（需有文字层）/ **图片截图**（由视觉模型识别成文字），生成时检索门票、预约、闭馆日、淡旺季等容易过时的信息拼进提示词；不开启也不影响主流程
